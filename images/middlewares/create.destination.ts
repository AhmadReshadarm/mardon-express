...
export async function createDestinationCompressed(req: Request, resp: Response, next: NextFunction) {
  if (!fs.existsSync(DESTINATION_COMPRESSED)) {
    try {
      await fs.promises.mkdir(DESTINATION_COMPRESSED);
      next();
    } catch (e: any) {
      if (e.errno !== -17) {
        throw new CustomExternalError([ErrorCode.INTERNAL_ERROR], HttpStatus.INTERNAL_SERVER_ERROR);
